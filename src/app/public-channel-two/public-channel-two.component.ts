...
export class PublicChannelTwoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.storage.currentImage.subscribe(image => {
      while (this.newImage) {
        this.storage.getImage(this.messageIdStorageTemp, this.channelId);
    ...
    this.storage.currentFile.subscribe(file => {
      while (this.newFile) {
        this.storage.getFile(this.fileTemp, this.messageIdStorageTemp, this.channelId);
    ...
    if (!this.uploadedFile) {
      if (file.type === 'image/jpeg' || file.type === 'image/png') {
        if (this.currentUser) {
          ...
          this.storage.putImage(file, messageId, this.channelId);
          this.checkFileName(file);
          this.fileTemp = file;
          this.messageIdStorageTemp = messageId;
          this.storage.getImage(messageId, this.channelId);
          this.newImage = true;
    ...
    if (!this.uploadedPhoto) {
      if (file.type !== 'image') {
        if (this.currentUser) {
          ...
          this.storage.putFile(file, messageId, this.channelId);
          this.fileNameMessage = file.name;
          this.checkFileName(file);
          this.fileTemp = file;
          this.messageIdStorageTemp = messageId;
          this.storage.getFile(file, messageId, this.channelId);
          this.newFile = true;
